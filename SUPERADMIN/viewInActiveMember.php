       ...
       <div class="iq-card">
          <div class="iq-card-header d-flex justify-content-between">
             <div class="iq-header-title">
                <h4 class="card-title">View InActive Member</h4>
             </div>
          </div>
          <div class="iq-card-body">
...
$count = 0;
$queryActive = mysqli_query($con, "SELECT user_id,name,phone,sponser_id,date_time,password,activation_date from sub_admin_user_details WHERE member_id!=1 AND topup_flag=0 AND user_type=2 AND CAST(date_time AS DATE) BETWEEN '$cal_date' AND '$cal_date1' ".$query.' ORDER BY date_time DESC');
while ($valActive = mysqli_fetch_assoc($queryActive)) {
    ++$count; ?>
                      <tr>
                        ...
                        <td><?php echo $valActive['phone']; ?></td>
                        <td><?php echo name($con, $valActive['sponser_id']).' (User ID:'.user_id($con, $valActive['sponser_id']).')'; ?></td>
                        <td><?php echo $valActive['date_time']; ?></td>
                      
                        <td><a href="viewMemberDetails?user_id=<?php echo $valActive['user_id']; ?>" class="btn btn-success btn-sm">More</a></td>
                      </tr>
                    <?php } ?>  
                   </tbody>
                </table>
...
var d = document.getElementById("member");
    d.className += " active";
var d = document.getElementById("viewInActiveMember");
    d.className += " active";
</script>
</body>
</html>