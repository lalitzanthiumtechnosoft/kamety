       ...
       <div class="iq-card">
          <div class="iq-card-header d-flex justify-content-between">
             <div class="iq-header-title">
                <h4 class="card-title">Weekly Bonus On Growth</h4>
             </div>
          </div>
          <div class="iq-card-body">
...
$count = 0;
$queryWeekly = mysqli_query($con, "SELECT b.name,b.user_id,a.* FROM user_weekly_income_details a, sub_admin_user_details b where a.member_id=b.member_id AND CAST(a.date_time AS date) BETWEEN '$cal_date' AND '$cal_date1' ".$query.' order by a.date_time desc');
while ($valWeekly = mysqli_fetch_assoc($queryWeekly)) {
    ++$count; ?>
                      <tr>
                        <td><?php echo $count; ?></td>
                        <td><?php echo $valWeekly['user_id']; ?></td>
                        <td><?php echo $valWeekly['name']; ?></td>
                        <td> <?php echo $valWeekly['weeklyIncome']; ?></td>
                        <td><?php echo $valWeekly['date_time']; ?></td>
                        <td> <?php echo $valWeekly['levelBonus'] + $valWeekly['shareBonus']; ?></td>
                        <td><?php echo $valWeekly['incomePercent']; ?></td>
                      </tr>
                    <?php } ?>  
                   </tbody>
                </table>
...
var d = document.getElementById("income");
    d.className += " active";
var d = document.getElementById("weeklyBonus");
    d.className += " active";
</script>
</body>
</html>