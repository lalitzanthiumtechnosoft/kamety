...
if (isset($_POST['addEditRemark'])) {
    ...
    $queryUpdate = mysqli_query($con, "UPDATE user_wallet_withdrawal_crypto SET remarks='$remarks',payment_date='$d',released='$withdrawStatus' WHERE id='$id'");
    if ($withdrawStatus == 3) {
        $queryAmount = mysqli_query($con, "SELECT amount FROM user_wallet_withdrawal_crypto WHERE id='$id'");
        $valAmount = mysqli_fetch_assoc($queryAmount);
        $amount = $valAmount['amount'];

        mysqli_query($con, "UPDATE sub_admin_user_details SET wallet=wallet+'$amount' WHERE member_id='$memberId'");
    ...
    if ($queryUpdate) { ?>
		<script>
			...
			window.top.location.href = 'walletWithdrawStatus?user_id=<?php echo $userId; ?>&withdrawStatus=<?php echo $withdrawStatus; ?>&from_date=<?php echo $fromDate; ?>&to_date=<?php echo $toDate; ?>';
		</script>
	<?php } else { ?>
		<script>
			alert("Action Taken Successfully!!!");
